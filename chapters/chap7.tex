...
\begin{theorema}[Legge di Gauss per la magnetostatica]\index{legge!di Gauss!per la magnetostatica}
	...
	\end{itemize}
\end{theorema}
La conseguenza immediata della legge di Gauss per il magnetismo è che il campo magnetostatico è un \textbf{campo solenoidale}\footnote{Nelle ‘‘XXX'', a pagina \ref{CampoSolenoidale} è possibile trovare la definizione di campo solenoidale e altre proprietà.}, a differenza del campo elettrostatico che è \textit{conservativo}.
\begin{digression}
...
\end{digression}
\subsection{L'esperimento di Ampére}\index{esperimento!di Ampére}
Anche il francese \textbf{Andre Marie Ampére} (1775-1836), che abbiamo incontrato in precedenza nel trattare la corrente elettrica, propose un esperimento analogo nel 1823. % - anche 
...
% TO DO: inserire disegno
Sperimentalmente, si trovò che la \textit{densità lineare} di forza era descritta dalla \textbf{legge di Ampère}\index{legge!di Ampère}
\begin{equation}
...
\subsection{Forza di Lorentz con campo magnetico uniforme: il caso perpendicolare alla velocità}
...
\end{equation}
\subsection{Forza di Lorentz con campo magnetico uniforme: il caso generale}
...
\end{equation}
\section{Seconda legge di Laplace e legge di Biot-Savart} % TO DO: BIOT-SAVART IS GONE
...
\end{observe}
\section{Il momento meccanico di un circuito piano in un campo magnetico}
...
Posta $\vba{r}$ la parametrizzazione di $\partial \Sigma$, si ha
	\begin{equation*}
		...
		\pdv{y} d\vba{s}=
		\pdv{z} d\vba{s}=0
		\end{equation*} % TO DO: perchè scusi?
...
\begin{cases}
	...
	\curl{d\vba{s}}=0
\end{cases}
\end{equation*} % TO DO: inserire curl esplicito
...
\end{observe} % TO DO: che osservazione del piffero.
...
\subsection{Campo magnetico generato da un filo}
...
\begin{define}[Seconda legge di Laplace]
	La \textbf{legge di Biot-Savart}\index{legge!di Biot-Savart}, detta anche \textbf{prima legge di Laplace}\index{leggi di Laplace!prima}, afferma che il campo magnetico prodotto da una corrente stazionaria $I$ che percorre un generico filo $\gamma$ è
	\begin{equation*}
...
\end{define}
\begin{examplewt}[Filo rettilineo di lunghezza {$2a$}]
	content
\end{examplewt}
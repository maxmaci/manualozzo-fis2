...
\end{equation*}
Tale campo produce un flusso \textit{non} nullo attraverso la superficie stessa, detto \textbf{autoflusso}\index{autoflusso}:
\begin{equation}
...
\begin{define}[Autoflusso]
	L'\textbf{autoflusso}\index{autoflusso} di una spira $\gamma$ percorsa da corrente $I$ e di induttanza $L$ è il flusso del campo magnetico generato dalla corrente tramite la superficie delimitata dalla spira stessa:
	\begin{equation}
...
Posta questa nomenclatura, l'idea è di modificare la legge di Ampère sostituendo alla densità di corrente $\vba{j}$ la \textbf{densità di corrente totale}\index{densità!di corrente!totale}
\begin{equation}
...
\begin{theorema}[Legge della circuitazione di Maxwell-Ampère]\index{legge!della circuitazione di Maxwell-Ampère}
	...
	\begin{itemize}
		...
		\begin{empheq}[box=\tcmathboxthm]{align}
			...
			&=\mu_0 I_{int}+\frac{1}{c^2}\pdv{\Phi_{\Sigma}(\vba{E})}{t}=\mu_0\left(I+I_s\right)s\label{LeggeAmpereIntegrale}
		\end{empheq}
		\item \textbf{Forma differenziale:}
		\begin{equation}
...
\begin{center}
	\begin{tabular}{p{0.18\textwidth}|c|c}
		...
		\begin{tabular}[l]{@{}l@{}}
			\\[-3mm]
			$\displaystyle\Gamma_{\gamma}(\vba{B})=\oint_{\partial \Sigma} \vba{B}\vdot d\vba{s}=\frac{1}{c^2}\dv{t} \int_{\Sigma}\vba{E}\vdot\vbh{u}_nd\Sigma=$\\
			$\displaystyle\quad=\frac{1}{c^2}\pdv{\Phi_{\Sigma}(\vba{E})}{t}=\mu_0I_s$
		\end{tabular} &
		\begin{tabular}[l]{@{}l@{}}
			\\[-2mm]
			$\displaystyle\curl{\vba{B}}=\frac{1}{c^2}\pdv{\vba{E}}{t}=\mu_0\vba{j}_s$
		\end{tabular}
	\end{tabular}
\end{center}
